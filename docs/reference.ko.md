...
아래쪽 패널은 탭을 눌러 명시적으로 고릅니다. `S/M/L`은 미터만·패널·상세 레이아웃이고,
`⋯`는 같은 항목과 오버레이 제어를 모은 전체 메뉴입니다.

| 패널 | 보이는 것 |
|---|---|
| **세션** (기본) | 상태 · 프로젝트 · 메인 tok/s · 누적 출력 · 컨텍스트. L은 모델·열린 시각·effort도 표시 |
| **한도** | 로그인된 Claude·Codex·Grok의 요금제 사용률·리셋 시각·갱신 상태 |
| **속도** | 서브에이전트를 제외한 메인 모델별 출력 처리량 |
| **일별 히스토리** | 최근 7일 날짜별 토큰·비용. 진행 중인 오늘이 맨 위에 함께 놓입니다 |
| **팀** (선택) | 자체 호스팅 endpoint의 확인·작업·대기 현황. endpoint가 오프라인이면 탭 자체를 숨깁니다 |
...
| ④ 항상 최전면에 전체 출력 처리량이 보인다 | `tokenmeter/overlay.py` — 28칸 세그먼트 게이지, 출력 토큰 델타 tok/s 지수평균에 연동된 채움·피크 홀드·감속 관성 | `python3 -m tokenmeter.demo` / `python3 -m tokenmeter.overlay`(미터 물리 자가 검증) |
| ⑦ 지나간 사용량을 되짚어 본다 | `tokenmeter/meter.py`가 하루가 끝날 때 `days`에 남기고, 오버레이 패널이 일별·세션별로 보여줍니다 | 패널 탭·`S/M/L`·`⋯` / `tokenmeter status` |
...
- 세션·한도·팀 탭은 L에서도 그래프 대신 더 많은 정보와 행을 보여줍니다
