...
오버레이는 드래그로 옮기고 휠로 세션 목록을 스크롤합니다. 화면의 `S/M/L`로 미터·패널·상세 크기를 바로 바꾸고, 세션·한도·속도·일별 탭과 세션 필터를 직접 선택합니다. `⌘K`/`Ctrl+K`는 선택형 빠른 검색입니다. 테마, 투명도·모션 감소는 `⋯`나 우클릭 메뉴에 있습니다. `×`는 오버레이만 숨기며 측정은 계속됩니다. 측정까지 끝내려면 메뉴나 트레이의 `TokenMeter 종료 · 측정 중지`를 고릅니다.
