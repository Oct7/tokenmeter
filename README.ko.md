...
오버레이는 드래그로 옮기고 휠로 세션 목록을 스크롤합니다. `⌘K`/`Ctrl+K`를 누르면 세션·사용량·한도·보기 명령을 바로 검색할 수 있고, `↑↓`와 `Enter`로 실행합니다. 크기, 테마, 투명도·모션 감소는 `⋯`나 우클릭 메뉴에 있습니다. `×`는 오버레이만 숨기며 측정은 계속됩니다. 측정까지 끝내려면 메뉴나 트레이의 `TokenMeter 종료 · 측정 중지`를 고릅니다.
